...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    /\.(xml|txt|ico|png|jpg|jpeg|svg|css|js)$/.test(pathname)
  ) {
  ...
  if (pathname === '/') {
    ...
    return NextResponse.redirect(new URL('/PUBG%20MOBILE/com.tencent.ig', request.url), 301);
  }
  ...
  if (pkgSegments.length === 1) {
    ...
    if (siteConfig && siteConfig.name) {
      ...
      // If the path isn't already the full one, redirect.
